/**
 * NAL: Inicializiraj polje 10 števil. Napiši funkcijo, ki sprejme 
 * polje števil in izračuna povprečno vseh števil v polju. Za 
 * shranjevanje povprečnega števila uporabi kazalce (pointerje).
 * 
 * V nadaljevanju je predstavljena osnovna struktura, ki vam bo v pomoč. 
 * 
 * Vaša naloga je dokončati funkcijo calc_avg.
 * 
 * IZPIS:
 * Povprecje: 15.900000
*/


#include <stdio.h>

void calc_avg(int numbers[10], double* p_avg);

int main() {
   
}


void calc_avg(int numbers[10], double* p_avg) {
    
}